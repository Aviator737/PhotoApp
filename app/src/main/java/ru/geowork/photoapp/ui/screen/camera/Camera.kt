...
fun Camera(
    ...
    var maxZoom by remember { mutableFloatStateOf(0f) }
    ...
    Column(modifier = Modifier.fillMaxWidth().background(BackgroundSecondaryDark)) {
        ...
        Row(
                ...
                if (it.isNotEmpty()) {
                    ...
                    inputJob = inputScope.launch {
                        ...
                        it.toFloatOrNull()?.let { float ->
                            cameraControl?.setZoomRatio(float)
                        }
                    }
                }
            }
            ButtonLarge(
                onClick = { cameraControl?.setZoomRatio(minZoom) }
            ) {
                Text(
                    text = minZoom.toString()+"x",
                    style = AppTheme.typography.semibold16
                )
            }
            ButtonLarge(
                onClick = { cameraControl?.setZoomRatio(2.0f) }
            ) {
                Text(
                    text = "2.0x",
                    style = AppTheme.typography.semibold16
            ...
            ButtonLarge(
                onClick = { cameraControl?.setZoomRatio(maxZoom) }
            ) {
                Text(
                    text = maxZoom.toString()+"x",
                    style = AppTheme.typography.semibold16
                )
            }
        }
        Text(
            text = "minZoomLevel: $minZoom",
            color = AppTheme.colors.contentConstant
        )
        Text(
            text = "maxZoomLevel: $maxZoom",
            color = AppTheme.colors.contentConstant
        )
